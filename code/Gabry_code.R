...
sigma_add =   rnorm(Nsim, mean = 0, sd = sigma)

data1 <- data.frame(
  log_pm25 = GM$log_pm25,
  sim = beta0 + beta0i[GM$super_region] +
    (beta1 + beta1i[GM$super_region]) * GM$log_sat + sigma_add)
)

plot(data1$log_pm25 ~ data1$sim)

var_ints = tibble(super_region = unique(GM$super_region)) %>% 
  mutate(beta0i = beta0i,
         beta1i = beta1i)

test <- tibble(log_pm25 = GM$log_pm25,
       log_sat = GM$log_sat,
       super_region = GM$super_region,
       beta0 = beta0,
       beta1 = beta1) %>%
  left_join(var_ints) %>% 
  mutate(sim = beta0 + beta0i + (beta1 + beta1i) * log_sat + sigma_add)
...
ggplot(test, aes(x = log_pm25, y = sim)) + 
  geom_point(alpha = 0.1, color = "red") + 
  xysim_labs
